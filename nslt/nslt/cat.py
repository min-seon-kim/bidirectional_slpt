...
def reply(output_text):
    # dir_name = "../Data/"
    # dir_name = "/home/minseon/SignLanguageTranslator/Assets/Resources/"
    dir_name = "../Data/Video/"

    tts = gTTS(text=output_text, lang='en')
    filename = dir_name + 'voice.mp3'
    tts.save(filename)
    open('../Data/predictions.en','w').close()
...
class Handler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        # Inferences
        print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        self.inference_fn(ckpt=ckpt, inference_input_file='../Data/test.sign', inference_output_file='../Data/predictions.de', hparams=hparams, num_workers=1, jobid=0, scope=None, single_cell_fn=None)
        # self.iter += 1

        # if (self.iter == 2):
        #     exit(0)
        # shutil.rmtree('../Data/Images', ignore_errors=True)
        # shutil.rmtree('../Data/ConvertedImages', ignore_errors=True)
        # os.mkdir("../Data/Images")
        # os.mkdir("../Data/ConvertedImages")
        with codecs.getreader("utf-8")(
            tf.gfile.GFile('../Data/predictions.en', "rb")) as f:
            output_text = f.read()
            ...
            # if idx == 1:
            #     output_text = "hello and good evening"
            # elif idx == 2:
            #     output_text = "new week starts to grow cool"
            # elif idx == 3:
            #     output_text = "good night everyone"
            # #####
            ...
            time.sleep(1)
            try:
                reply(output_text.encode('utf-8')) # create video mp3
            except:
                for file in scandir.scandir('../Data/Images'):
                    try:
                        os.remove(file.path)
                    except:
                        pass
                for file in scandir.scandir('../Data/ConvertedImages'):
                    try: 
                        os.remove(file.path)
                    except:
                        pass
...
def run_main(flags, default_hparams, target_session=""):
    """Run main."""
    # Job
    jobid = flags.jobid
...
if __name__ == "__main__":
    ...
    global ckpt
    ckpt = tf.train.latest_checkpoint("../Output")
    print('------------------ckpt--------------')
